#ifndef FT_SET_TESTER_HPP
# define FT_SET_TESTER_HPP
...
class SetTestCase<Container, int>
{
public:
    typedef typename Container::iterator    iterator;
	...
	void equal_range()
	{
		this->ret = *container.equal_range(1).first;
	}
    Container& get_container()
    { return container; }
...
class SetTester : public TesterBase
{
public:
    typedef typename std::set<Key>            			master;
    typedef typename ft::Set<Key>             			challenger;
    typedef typename std::set<Key>::iterator  			master_iterator;
    typedef typename ft::Set<Key>::iterator  			challenger_iterator;
	typedef typename std::set<Key>::reverse_iterator	master_reverse_iterator;
	typedef typename ft::Set<Key>::reverse_iterator		challenger_reverse_iterator;

    SetTester() : TesterBase("Set")
	{ init_what_to_test(); }

    SetTester(bool log_flag, std::string dir = "")
        : TesterBase("Set", log_flag, dir)
	{ init_what_to_test(); }
...
private:
    ...
    SetTestCase<master, Key>     	master_case;
    SetTestCase<challenger, Key>  	challenger_case;
};

#endif /* FT_SET_TESTER_HPP */
